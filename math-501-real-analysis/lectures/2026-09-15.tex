...
\begin{example}[A fat Cantor set]\label{ex:fat-cantor-sep-15}
  Set $K_0=[0,1]$. At stage $n\geq1$, remove from the center of each
  interval component of $K_{n-1}$ an open interval of length $4^{-n}$,
  and call the remaining set $K_n$. Thus the first step removes the
  middle interval $(3/8,5/8)$ and leaves
  \[
    K_1=\left[0,\frac38\right]\cup
        \left[\frac58,1\right].
  \]
  There are $2^{n-1}$ intervals removed at stage $n$, each of length
  $4^{-n}$. Hence, for the limit set $K=\bigcap_{n=0}^{\infty}K_n$,
  \[
    m(K)
    =1-\sum_{n=1}^{\infty}2^{n-1}4^{-n}
    =1-\sum_{n=1}^{\infty}2^{-n-1}
    =\frac12.
  \]
  The lengths of the interval components tend to zero, while every
  component splits into two components at the next stage. Consequently,
  $K$ is compact, perfect, has empty interior, and is totally
  disconnected. Thus $K$ has all the topological features of the usual
  Cantor set but has positive measure. It is called the
  \emph{fat Cantor set} (or the \emph{Smith--Volterra--Cantor set}).

  \begin{figure}[htbp]
    \centering
    \begin{tikzpicture}[x=11cm,y=0.82cm,
      kept/.style={blue!70!black,line width=3pt,line cap=round},
      endpoint/.style={fill=blue!70!black,inner sep=1.15pt,circle}]
      \foreach \y/\lab in {0/{K_0},-1/{K_1},-2/{K_2},-3/{K_3}} {
        \node[anchor=east] at (-0.035,\y) {$\lab$};
      }

      \draw[kept] (0,0)--(1,0);

      \foreach \a/\b in {0/.375,.625/1} {
        \draw[kept] (\a,-1)--(\b,-1);
      }

      \foreach \a/\b in {0/.15625,.21875/.375,.625/.78125,.84375/1} {
        \draw[kept] (\a,-2)--(\b,-2);
      }

      \foreach \a/\b in {
        0/.0703125,.0859375/.15625,
        .21875/.2890625,.3046875/.375,
        .625/.6953125,.7109375/.78125,
        .84375/.9140625,.9296875/1} {
        \draw[kept] (\a,-3)--(\b,-3);
      }

      \draw[red!70!black,<->,>=Latex] (.375,-.55)--(.625,-.55)
        node[midway,fill=white,inner sep=1pt] {$4^{-1}$};
      \draw[red!70!black,<->,>=Latex] (.15625,-1.55)--(.21875,-1.55)
        node[midway,fill=white,inner sep=1pt] {\scriptsize$4^{-2}$};

      \foreach \x/\lab in {0/0,.375/{\frac38},.625/{\frac58},1/1} {
        \draw (\x,-3.12)--(\x,-3.24)
          node[below=1pt] {$\lab$};
      }
    \end{tikzpicture}
    \caption{The first stages of the fat Cantor construction. The
      deleted intervals become shorter quickly enough that the limiting
      set retains measure $1/2$.}
    \label{fig:fat-cantor-sep-15}
  \end{figure}
\end{example}
